...
- **client apps** — `cli`, `web`;
- `api` — public entrypoints for client-facing HTTP APIs and SSE streams;
...
- `api` send live updates to clients over **SSE**.
...
- `api` is the only public entrypoints for client-facing HTTP and SSE communication.

...
Platformik is an internal tool (showcase-quality, open-source) for building and running **AI agent pipelines**:
...
- Users can plug in multiple LLM providers (OpenAI, Anthropic, etc.), choose models per flow, and optimize token usage.
...
- LLMs never directly mutate canonical memory; they output `proposed_memory_patch` which is validated before commit.

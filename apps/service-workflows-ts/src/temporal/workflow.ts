...
} from '@platformik/module-workflows-ts/contracts'
import type { createActivities } from './activities'

const { deliverMessage, presentOptionInput, completeRun } = proxyActivities<
...
export async function interactiveDslWorkflow(definition: FlowDefinition): Promise<void> {
  ...
  // Sequence 1 is reserved for run_started (inserted by the gRPC handler).
  ...
  while (currentNodeId !== undefined) {
    ...
    if (node.type === 'option_selection') {
      ...
      const activityOptions = nodeOptions.map((o, i) => ({ id: String(i + 1), label: o.label }))
      eventSequence++
      await presentOptionInput(runId, node.question, activityOptions, eventSequence)

      pendingQuestion = node.question
      pendingOptions = nodeOptions.map((o) => o.label)
      ...
      while (selectedOption === undefined) {
        ...
        if (selectedOption === undefined) {
          const hint = nodeOptions.map((o, i) => `${i + 1}. ${o.label}`).join(', ')

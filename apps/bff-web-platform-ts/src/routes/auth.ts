...
import { applyHeadersToReply, parseHeaders } from '../ops/http/headers'
...
export const authWriteRoutePaths = new Set(['/auth/sign-up', '/auth/sign-in', '/auth/sign-out'])
...
export const registerAuthRoutes = async (
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      const result = await options.iam.auth.signUp(body, parseHeaders(request.headers))
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      const result = await options.iam.auth.signIn(body, parseHeaders(request.headers))

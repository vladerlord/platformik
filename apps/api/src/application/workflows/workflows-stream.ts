...
export const mapRunViewToSseDeltas = (
  view: Awaited<ReturnType<StreamWorkflowEventsContext['workflowsService']['getWorkflowRunView']>>,
  ...
  const terminal =
    view.status === 'WORKFLOW_RUN_STATUS_COMPLETED' || view.status === 'WORKFLOW_RUN_STATUS_FAILED'
...
export const streamWorkflowEvents = async function* (
  ...
  const hydrateRunView = async (): Promise<
    Awaited<ReturnType<StreamWorkflowEventsContext['workflowsService']['getWorkflowRunView']>>
  > => {
    const params = {
      workflowRunId: context.runId,
      context: context.authContext,
      ...(lastMessageId !== undefined ? { afterId: lastMessageId } : {}),
    }

    return context.workflowsService.getWorkflowRunView(params)
  }
